...
def plot_match_summary(ax, home_team, away_team, venue, city, season, round_str, match_date, match_time, score, logo_file_path):
    ...
    home_name_text_props =[
        {
            "bbox": {"edgecolor": home_primary_colour, "facecolor": home_primary_colour, "linewidth": 1.5, "pad": 2},
            "color":"w",
            "fontsize":24
        }
    ]
    HighlightText(x=0.15, y=0.8,
                ha='center', va='center',
    ...
    away_name_text_props = [
        {
            "bbox": {"edgecolor": away_primary_colour, "facecolor": away_primary_colour, "linewidth": 1.5, "pad": 2},
            "color":"w",
            "fontsize":24
        }  
        ]
    HighlightText(x=0.85, y=0.8,
                ha='center', va='center',
    ...
    HighlightText(x=0.5, y=0.4,
                ha='center', va='center',
                s=f'<{score}>',
                highlight_textprops=score_text_props,
                fontname = "DM Sans",
                ax=ax)
...
def plot_match_information(ax, match_id, summary, logo_file_path):
    ...
    ax = plot_match_summary(ax, home_team, away_team, venue, city, season, round_str, match_date, match_time, score, logo_file_path)
