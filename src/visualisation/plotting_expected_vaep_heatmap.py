...
def plot_heatmap(pitch, ax, relative_value_statistic, match_chains, custom_cmap, bins = (4, 5)):
    ...
    pitch.label_heatmap(plotting, ax=ax, str_format='{:.0f}',
                            color='white', fontsize=6, va='center', ha='center',
                            )
    
    return pitch, ax

def plot_expected_vaep_heatmap(ax, chain_data, match_id):
    ...
    relative_value_statistic = calculate_heatmap_statistics(pitch, home_chains, away_chains, bins = (4, 5))
    home_team, away_team = get_teams(match_id)
    custom_cmap = create_colourmap(team_colours, home_team, away_team)
    
    pitch, ax = plot_heatmap(pitch, ax, relative_value_statistic, home_chains, custom_cmap, bins = (4, 5))
